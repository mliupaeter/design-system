...
<StorybookExample componentName="card" storyId="components-card--actionable" />
...
- Use `variation="actionable"` only when the card contains a semantic action, such as a link or button. The motion and visual cue are supplemental; the link or button must remain visible and keyboard accessible.
- Actionable card motion respects reduced-motion preferences and should not be used to imply urgency.
